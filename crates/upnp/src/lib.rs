...
impl UpnpEndpoint {
    ...
    fn get_wan_ip_control_urls(&self) -> impl Iterator<Item = (tracing::Span, Url, String)> + '_ {
        self.iter_services()
            ...
            .filter_map(|(span, url, st)| match url {
                Ok(url) => Some((span, url, st)),
                Err(e) => {
                    ...
                    if control_urls.is_empty() {
                        debug!(
                            ...
                            "no WANIPConnection services matched this endpoint; not forwarding. \
                             Supported service types: WANIPConnection:1, WANIPConnection:2"
                        );
                    }
                    let mut local_ip = None;
                    for (span, control_url, service_type) in control_urls {
                        ...
                        debug!(%control_url, %ip, service_type = %service_type, "spawning port manager");
